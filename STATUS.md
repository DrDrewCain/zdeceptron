...
**`examples/` holds thirty-three files. All thirty-three check and all thirty-three
build.** Thirty-one sit directly in `examples/` and two — `tree/` and `tree-webgl/` —
are in a directory of their own because each has assets beside it. Thirty-two of the
thirty-three are programs; the odd one out is named below.
