...
| `examples/tree/tree.zd` | ✅ | ✅ | The one example in a directory of its own, because it has files beside it. The tree's *structure* is derived in ZDeceptron — four parallel `List of Whole`, since §14E.1 lets a view `foreign` take only `Text`/`Whole`/`Decimal`/`Truth` and lists of those, not records — and the *geometry* is three.js in `draw.js` behind a `foreign … gives view`. Tailwind arrives as `assets/tailwind.css` under the §6.1 `class is` rule. `build` ships `draw.js` and the asset beside `client.js`. |
