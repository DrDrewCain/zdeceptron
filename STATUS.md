...
| Crate | Tests | Note |
|---|---|---|
| `zdc-codegen` | 1000 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out. |
...
| `zdc-bench` | 63 | Plus 3 ignored. Includes the exact-match `BENCHMARKS.md` gate. |
...
| `zdc-hir` | 40 | |
| `zdc-runtime` | 89 | Two of these run the JavaScript suites — further assertions the count above does not see. `tests/wire_version.rs` pins the wire format's version across its three spellings (#144). |
| `zdc-ast` | 18 | |
