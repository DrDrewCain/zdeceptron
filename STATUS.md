...
| **M1** | Indentation-sensitive lexer + parser + AST, snapshot tests | ✅ **done** | `zdc-lexer` 63 tests including `src/layout.rs`; `zdc-parser` 149 across boundary-focused files; `zdc-ast` 4. `zdc parse examples/hello.zd` exits 0. The spec's testing table asks for `insta` snapshot tests and this project does not use them; that is no longer a standing deviation but a decision, argued in [`DECISIONS.md` §4](DECISIONS.md), and this row no longer restates it. |
