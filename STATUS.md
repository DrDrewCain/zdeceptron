...
across **21 crates** — re-taken on `feature/zdc-fmt`, which adds a crate and its
tests. See [§3](#3-tests) for how, why the flag is not optional, and which of the per-crate
rows below moved.
>>>>>>> c0cf896 (Count the crates and the gates, and then stop counting them by hand)
...
=======
| **M0** | Repository, workspace, CI, spec | ✅ **done** | **21-crate** Cargo workspace — `zdc-wasm` is the twenty-first (#171). `.github/workflows/ci.yml` runs `fmt --check`, `clippy -D warnings`, `zdc fmt --check` over every `.zd` file under `examples/`, and **nine** scripted gates: `check-forbid-unsafe.sh`, `check-wildcard-arms.sh`, `check-vacuous-tests.py`, `check-emitted-strings.sh`, `check-grammar-drift.py`, `check-message-budget.py`, `check-advisory-exceptions.sh`, `check-installer.sh`, and `check-dependency-unsafe.sh` via `cargo-geiger`, plus `cargo deny` and `cargo audit`. `cargo test --workspace --no-fail-fast` is a CI step. |
>>>>>>> c0cf896 (Count the crates and the gates, and then stop counting them by hand)
...
| **M0** | Repository, workspace, CI, spec | ✅ **done** | **20-crate** Cargo workspace — `zdc-fmt` is the twentieth (#167). `.github/workflows/ci.yml` runs `fmt --check`, `clippy -D warnings`, `zdc fmt --check` over every `.zd` file under `examples/`, and **eight** scripted gates: `check-forbid-unsafe.sh`, `check-wildcard-arms.sh`, `check-vacuous-tests.py`, `check-emitted-strings.sh`, `check-grammar-drift.py`, `check-advisory-exceptions.sh`, `cargo deny`, `cargo audit`, plus `check-dependency-unsafe.sh` via `cargo-geiger`. `cargo test --workspace --no-fail-fast` is a CI step. |
| **M1** | Indentation-sensitive lexer + parser + AST, snapshot tests | ✅ **done** | `zdc-lexer` 96 tests including `src/layout.rs`; `zdc-parser` 206 across boundary-focused files; `zdc-ast` 12. `zdc parse examples/hello.zd` exits 0. **The spec's testing table asks for `insta` snapshot tests and this project does not use it — decided 2026-08-16 (#157), not deviated from.** It was tried: `insta` was a dev-dependency of `zdc-lexer`, `zdc-parser` and `zdc-cli`, and produced one snapshot — 853 lines of `voting-board.zd`'s printed AST, deleted with the dependency in `87b1b5d` for a test that names the declarations, the pipeline clauses and the handler it cares about. A snapshot's expected value is written by the code under test, so it cannot be written first and cannot be watched to fail; `layout.rs`'s `nesting_emits_indent_and_dedent` is the indentation edge case that table meant, and its expectation is a token vector somebody wrote by hand. [`CONTRIBUTING.md`](CONTRIBUTING.md#snapshot-tests-and-the-dependency-this-project-does-not-take) carries the argument and what reverses it. |
...
| **M7** | `durable` placement, store, SSE sync | ✅ **done — one deviation, one named gap** | `zdc-store` (63 tests), a durable store over one total order; `runtime/store.js` and `runtime/wire.js` are the browser half; live sync over a transport seam, `streamTransport` and `pollTransport`. **Evidence is `crates/zdc-host/tests/two_windows.rs` (7 tests):** one window increments, the other is told the new value with no round trip, a reconnecting window is replayed what it missed, and two windows over a reopened database agree. **Deviation:** the store is `redb`, not SQLite — chosen because SQLite would link a C library and forfeit §7's single static binary. **Two questions that deviation raises are now decided rather than open (2026-08-16).** *Related data* (#36): no relational engine, because `watch` takes a key set and a key set is affordable only while every durable key is a declaration — a query is a set of keys that is not one. Predicates, joins, sorting on a computed key, aggregation and grouping are already writable as a `function` over a pipeline; the missing thing is an index. *A second deploy* (#37): a `durable` declaration you add needs no migration and never did, and one you retype is silent corruption — **measured: the same program built with `durable Whole` and with `durable Text` produces byte-identical `manifest.json` files**, and the store keeps one bookkeeping row, the sequence counter. A shape digest is adopted and **not yet built**; a migration is refused with a stated order. `docs/reference.md` carries both arguments and what reverses each, under [*Querying related data*](docs/reference.md#querying-related-data) and [*What a second deploy does to data the first one wrote*](docs/reference.md#what-a-second-deploy-does-to-data-the-first-one-wrote). |
...
| `examples/schedule.zd` | ✅ | ✅ | The other half of the clock: `every "1h"` on a `server` declaration is §14G.4's scheduled job, and the block under it is a real server root at `(Server, Trigger)` (#18). The cell holds the beat's *scheduled* start time in seconds, so a skipped beat shows as a jump larger than the cadence. Nothing on the wire may start it: a job is absent from `_zd/endpoints.js`, from the dev server's endpoint map and from the host's. `zdc deploy --target cloudflare` writes the cron rule and a `scheduled()` export; the other three targets **refuse this file**, each naming the platform fact that stops them. |
...
=======
**2358 passing, 0 failing, 9 ignored**, across 21 crates and 141 test binaries plus 19 doc-test
...
the total had grown from about 1,546 to 2,661. The per-crate rows below are
gated against the tree, so their sum is the workspace total and can be
re-added at any time; it is deliberately not written out here, because a
total in prose is a number nobody re-derives.
...
| `zdc-cli` | 158 | Re-counted here. End-to-end over the real binary, including a seeded fuzz harness and `tests/fmt_examples.rs`, which mangles every example, lays it out again and compares the emitted bundle byte for byte. |
=======
| `zdc-codegen` | 953 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out. |
| `zdc-types` | 234 | Plus 2 ignored, both recording an open language decision. |
| `zdc-parser` | 220 | Split across boundary-focused files. |
...
| 6 | **Medium** | `crates/zdc-host/src/bindings.rs`'s driver, `crates/zdc-deploy/js/router.js` | **A request body is never decoded, on either runner.** `rpc.js` posts `stringify(args)` — wire-encoded — and both server halves read it back with plain JSON: `$zdArgs = JSON.parse(…)` in the driver, `args = await request.json()` in the router. A value endpoint declaring a `Map of K to V` parameter is therefore handed `{"$map":[…]}` rather than a `Map`. The two runners agree, which is why no `zdc dev`-against-deployment divergence test sees it, and today's emitted code cancels the error out: a command's argument goes straight to `$store.set`, and both store layers write the cell in wire form regardless. The bytes come out right by cancellation, not by design. **It cannot be fixed in one file.** Decoding on one runner alone is the same divergence pointing the other way; decoding on both hands a real `Map` to three adapter stores that write their cell with `JSON.stringify`, which is #204 one layer lower — so the fix is both runners, the three stores' cell serialisation and `cells.js`'s `removed` at once, and it is ordered behind `fix/deploy-router-uses-the-wire-format`, because a store that returns a `Map` needs a router that answers in the wire format. Issue #336, part 1; part 2 of the same issue is fixed on this branch. |
