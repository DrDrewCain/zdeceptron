...
| **M5** | JS codegen + runtime; client-only programs run in a browser; benchmark suite in CI | ✅ **done**, except the React/Solid arm | `zdc-codegen` 834 tests, `zdc-runtime` 58 (which execute `runtime/signal.test.js` and `runtime/dom.test.js` under an embedded pure-Rust JS engine), `zdc-bench` 53 (plus 4 ignored: three surveys and one timed sweep). `BENCHMARKS.md`'s generated region (lines 119–240) is regenerated from the suite and exact-match gated, and `ci.yml`'s `asymptotics` job gates the one thing an exact-match on bytes cannot see — a pass getting slower without its output changing (#165). **Not delivered:** §14A.4's React and SolidJS arms, which need a package manager CI does not have. |
...
| **M8** | Style compilation to static CSS | ✅ **done** | `styles.rs` interns one class per *distinct* declaration set and emits `styles.css` as `runtime/base.css` plus generated rules; signal-dependent styles become `bindStyle`. **The surface is no longer small: 45 style arguments (`elements.rs::STYLE_ARGUMENTS`) plus 18 global ones, each with a value grammar in `crates/zdc-codegen/src/style.rs`, and 42 of them take any of seven conditional prefixes** (`hover`, `focus`, `active`, `disabled`, `narrow`, `wide`, `dark`) **or three keyframe steps** (`from`, `mid`, `to`), so one class carries its own `:hover`, breakpoint, `prefers-color-scheme` rules *and* its `@keyframes` block, and the interning property still holds. Tests: `class_and_style.rs` 8, `injection.rs` 28, `style_vocabulary.rs` 131, `styles.rs` 12 unit, `style.rs` 8 unit. **Verified by building:** `zdc build examples/todo.zd` emits `text-decoration-line: line-through` for a done item, which is the one visual state the canonical benchmark is about and could not previously render. A three-animation page emits three `@keyframes` blocks and three rules, every one of them inside `@media (prefers-reduced-motion: no-preference)`, and Chrome 151 reports `running=3 | names=zd-k1,zd-k2,zd-k3` for it and `name=none | running=0` under `--force-prefers-reduced-motion`. `runtime/base.css` is 3,641 bytes, up from 927. |
...
| Crate | Tests | Note |
|---|---|---|
| `zdc-codegen` | 933 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out. |
| `zdc-types` | 233 | Plus 2 ignored, both recording an open language decision. |
| `zdc-parser` | 206 | Split across boundary-focused files. |
| `zdc-graph` | 207 | Including the information-flow negative suite and the failure channel. |
