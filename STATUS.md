...
| Crate | Tests | Note |
|---|---|---|
| `zdc-codegen` | 872 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out, and `tests/showing_a_truth.rs`, the eight that pin which word a `Truth` reaches the page in (#297). |
