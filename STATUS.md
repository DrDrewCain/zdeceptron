...
| `zdc-cli` | 139 | Re-counted here. End-to-end over the real binary, including a seeded fuzz harness; `tests/fmt_examples.rs`, which mangles every example, lays it out again and compares the emitted bundle byte for byte; and `tests/deploy.rs`, which now asserts a deployment carries the asset directory and not only the pages. |
