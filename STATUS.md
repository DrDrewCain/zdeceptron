...
| **M5** | JS codegen + runtime; client-only programs run in a browser; benchmark suite in CI | ✅ **done**, without the React/Solid arm | `zdc-codegen` 948 tests, `zdc-runtime` 58 (which execute `runtime/signal.test.js` and `runtime/dom.test.js` under an embedded pure-Rust JS engine), `zdc-bench` 50 (plus 3 ignored surveys). `BENCHMARKS.md`'s generated region is regenerated from the suite and exact-match gated. **Not built, and decided 2026-08-16 (#158) rather than owed:** §14A.4's React and SolidJS arms. The reason this row used to give was wrong — CI has a network, and §8 governs emitted functions rather than the test suite. What holds is that the suite is an ordinary workspace test with nothing else installed, and that an arm run against a 506-line DOM shim written to answer this runtime's calls would produce a number about the shim. [`BENCHMARKS.md`](BENCHMARKS.md#the-two-arms-that-are-not-here) carries the argument and names what reverses it. |
...
**`examples/` holds thirty-eight files. All thirty-eight check and all thirty-eight
build.** Thirty-six sit directly in `examples/` and two — `tree/` and `tree-webgl/` —
are in a directory of their own because each has assets beside it. Thirty-seven of the
thirty-eight are programs; the odd one out is named below.
...
| `zdc-cli` | 154 | Re-counted here. End-to-end over the real binary, including a seeded fuzz harness and `tests/fmt_examples.rs`, which mangles every example, lays it out again and compares the emitted bundle byte for byte. `tests/editor_configs.rs` holds every configuration in `editors/` to the subcommand it launches, which is the one kind of breakage an editor reports as an absence rather than an error. |
=======
| `zdc-codegen` | 948 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out. |
| `zdc-types` | 233 | Plus 2 ignored, both recording an open language decision. |
| `zdc-parser` | 220 | Split across boundary-focused files. |
...
| `zdc-dev` | 121 | Self-contained unit suites plus integration files driving the running server. |
| `zdc-lsp` | 179 | Re-counted when `zdc doc` landed. |
| `zdc-cli` | 154 | Re-counted here. End-to-end over the real binary, including a seeded fuzz harness and `tests/fmt_examples.rs`, which mangles every example, lays it out again and compares the emitted bundle byte for byte. |
>>>>>>> ca4449e (Drive zdc dev from a real browser, in the suite)
=======
...
| `zdc-resolve` | 178 | Includes the `use`-sandbox suite, the two tests that pin what a `use` specifier may identify (#174), and the instantiation bounds. |
...
| `zdc-cli` | 139 | Re-counted here. End-to-end over the real binary, including a seeded fuzz harness and `tests/fmt_examples.rs`, which mangles every example, lays it out again and compares the emitted bundle byte for byte. |
>>>>>>> d4f880b (Decide how one program depends on another: it is a file)
...
| `zdc-deploy` | 55 | Four platform adapters and the portability claim. `tests/wire_format.rs` runs the generated router beside `zdc-host` and requires the two to answer the same bytes. |
