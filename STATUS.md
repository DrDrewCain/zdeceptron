...
| **M5** | JS codegen + runtime; client-only programs run in a browser; benchmark suite in CI | ✅ **done**, except the React/Solid arm | `zdc-codegen` 884 tests, `zdc-runtime` 59 (which execute `runtime/signal.test.js` and `runtime/dom.test.js` under an embedded pure-Rust JS engine), `zdc-bench` 53 (plus 3 ignored surveys). `BENCHMARKS.md`'s generated region (lines 119–240) is regenerated from the suite and exact-match gated. **Not delivered:** §14A.4's React and SolidJS arms, which need a package manager CI does not have. |
...
| Crate | Tests | Note |
|---|---|---|
| `zdc-codegen` | 884 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out; `tests/showing_a_truth.rs`, the eight that pin which word a `Truth` reaches the page in (#297); `tests/scene_sizing.rs`, the five that pin a drawing's box to the coordinate space it declares; and `tests/without_scripting.rs`, the four that hold a page's fallback to appearing exactly when the page needs it (#141) and keep the first paint off a keystroke's path. |
...
| `zdc-hir` | 40 | |
| `zdc-runtime` | 74 | Two of these run the JavaScript suites — further assertions the count above does not see. Fifteen are #135's minifier: the tokeniser's hazards (a regex literal with `/` in a character class, `//` inside strings and template literals, nested `${…}`, automatic semicolon insertion across a block comment), and the two properties that make it safe — minifying only ever removes characters, and the release build of every module still runs. |
| `zdc-ast` | 12 | |
