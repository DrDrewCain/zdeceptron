...
| Crate | Tests | Note |
|---|---|---|
| `zdc-codegen` | 878 | The largest suite, and the only row re-measured on this branch. Includes `tests/algorithms.rs`, the 19 tests that run the six algorithm examples and read their answers back out; `tests/showing_a_truth.rs`, the eight that pin which word a `Truth` reaches the page in (#297); `tests/scene_sizing.rs`, the five that pin a drawing's box to the coordinate space it declares; and `tests/without_scripting.rs`, the three that hold a page's fallback to appearing exactly when the page needs it (#141). |
