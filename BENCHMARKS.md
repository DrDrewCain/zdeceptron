...
| Asked for | Status |
|---|---|
| React and SolidJS | Not built, and **decided** rather than deferred — [`DECISIONS.md` §5](DECISIONS.md). The reason this row used to give was wrong in one half: CI plainly *has* a network, since `.github/workflows/ci.yml` fetches actions and crates on every run. The two real constraints are that the project's own claim is that it installs nothing — `release.yml` opens with *"no Node, no npm"* — and that this harness runs in `boa` against a counting DOM shim, which `react-dom`'s scheduler and synthetic event system would need a second, much larger and unverified DOM implementation to survive. The vendoring alternative (browser bundles committed to the tree, counts being engine-independent) is priced and refused there. **Nothing here is a measurement against React or Solid.** In their place stand the code-generator design §16.1 rejected, and hand-written JavaScript in two styles. |
