...
///
/// The sink list was not extended, because there was nothing to extend it
/// in: when this was written `zdc-graph` did not exist and codegen refused
/// any `secret` signal outright. The pass has landed, and all three are
/// still compile errors — now for the reason they should be: §5.3 refuses
/// a `secret client` declaration outright, and the flow pass reports the
/// path to the view beside it. What is *still* outstanding is the third
/// case's sink: a URL a browser fetches is not one of §17.3.1's four, and
/// nothing here can pin a rule the specification has not declared.
#[test]
fn a_secret_reaching_an_attribute_is_a_compile_error() {
    for source in [
        ...
        let messages = refusals(source);
        assert!(
            messages.iter().any(|m| m.contains("`secret`")),
