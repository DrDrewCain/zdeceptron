//! The two halves of the URL rule, run against one table.
//!
//! `zdc_hir::url_is_safe` settles every URL the compiler can see; `safeUrl`
//! in `runtime/dom.js` settles the ones it cannot, because they are not
//! known until the program runs. Two allowlists that can drift are one
//! allowlist too many: a scheme added to the Rust side and not to the
//! JavaScript side is a URL the compiler admits and the runtime blanks, and
//! a scheme added to the JavaScript side and not to the Rust side is a
//! `javascript:` hole with a test suite that says otherwise.
//!
...
const CASES: &[(&str, bool)] = &[
    ...
    // scheme. `/a:b` is a relative URL and refusing it would refuse a file
    // whose name contains a colon.
    ("/a:b", true),
    ("?q=a:b", true),
    ("#a:b", true),
    ...
    // are spelled, because the browser does not care about the case either.
...
/// What `safeUrl` did to `url`: it returns the URL unchanged when it is
/// safe and the empty string when it is not.
///
/// Comparing against the input rather than against `''` is what makes the
/// empty URL — safe, and unchanged — read correctly instead of as a
/// refusal.
...
/// The two allowlists themselves, not merely their consequences on a table.
///
/// A scheme added to one side and not to the other is caught by the cases
/// above only if somebody remembers to add a case. This is caught either
/// way.
#[test]
fn the_two_allowlists_hold_the_same_schemes() {
    let mut context = context(false);
    let listed = context
