...
use support::{compile_source, foreign_context, refusals, resolve_refusals, run};
...
///
/// `foreign_context` and not `context`: the lifecycle ships as its own
/// module (`runtime/foreign.js`), so a program that writes no `gives view`
/// foreign never downloads it. These tests are the programs that do.
