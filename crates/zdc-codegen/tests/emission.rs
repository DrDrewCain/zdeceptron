...
use support::{codegen_refusals, compile_example, compile_source, context, resolve_refusals, run};
...
fn assert_refused(source: &str, needle: &str) {
    let messages = codegen_refusals(source);
    assert!(
...
fn asset_stylesheets_are_linked_after_the_generated_one() {
    ...
    let table = zdc_types::check(&hir, &split).expect("typechecks");
