...
/// escape there, and `manifest.json` is read by `JSON.parse` rather than
/// by an evaluator. Every C0 control must be escaped rather than merely
/// being unwise.
...
/// and after this it does not type-check either.
///
