...
use zdc_ast::{
    ...
    RouteDecl, RouteParamDecl, RouteVariantDecl, StateDecl, TypeExpr, UseDecl, VariantDecl,
};
...
impl Parser {
    pub fn state_decl(&mut self) -> Result<StateDecl, ParseError> {
        ...
        // other. They are accepted in either order for the same reason
        // §4.1 gives one phrasing per construct: neither order is the
        // "wrong" one to a reader, and rejecting one would be a rule with
        // no meaning behind it.
        let mut secret = self.eat(&TokenKind::Secret);
        let trusted = self.eat(&TokenKind::Trusted);
        if !secret {
            secret = self.eat(&TokenKind::Secret);
        }
...
mod tests {
    ...
    fn trusted_and_secret_are_independent() {
        assert!(state("trusted state a is durable Text starting \"\"").trusted);
        let both = state("secret trusted state a is durable Text starting \"\"");
        assert!(both.trusted && both.secret);
        let other_order = state("trusted secret state a is durable Text starting \"\"");
        assert!(other_order.trusted && other_order.secret);
    }
