...
use zdc_ast as ast;
use zdc_hir::{
    Builtin, BuiltinVariant, Choice, Def, DefId, DefKind, ExprId, Field, Foreign, Function, Hir,
    HirArg, HirArm, HirArmBody, HirBind, HirBinding, HirBlock, HirEach, HirEachNode, HirElement,
    HirExpr, HirExprKind, HirHandler, HirIf, HirMutation, HirNode, HirNodeArm, HirNodeArmBody,
    HirPathSeg, HirPipeline, HirPlace, HirStmt, HirWhen, HirWhenNode, Local, LocalId, OperatorName,
    Record, Res, Signal, Variant, View, BUILTIN_OF_OPERATORS,
};
...
impl<'a> Resolver<'a> {
    ...
    pub fn resolve(mut self) -> Result<Hir, Vec<ResolveError>> {
        ...
        self.globals = collect(&decls, self.prelude)?;
    ...
    /// The top-level meaning of a name: a declaration, a declared variant,
    /// or one the language provides.
    fn global_name(&mut self, name: &str) -> Option<Res> {
        if let Some(index) = self.globals.lookup(name) {
