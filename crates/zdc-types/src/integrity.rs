...
impl<'a> Pass<'a> {
        ...
        match &self.hir.exprs[id].kind {
            ...
            HirExprKind::Call { callee, args } => {
                ...
                match callee {
                    ...
                    Res::Local(_) | Res::Builtin(_) | Res::Variant { .. } => label,
                }
            }
    ...
    fn reference(&mut self, res: Res, span: Span) -> Label {
        match res {
            Res::Builtin(_) | Res::Variant { .. } => Label::trusted(),
            Res::Local(local) => {
    ...
    fn mutation(&mut self, mutation: &HirMutation, pc: Label) {
        ...
        if pc.is_untrusted() {
            self.report(
                format!(
                    "`{name}` is `trusted`, and this write happens only when a browser-chosen \
                     value decided it should. That is a browser choosing the contents by another \
                     route, because {} (E-INT-04).",
                    pc.reason()
                ),
                place.span,
                None,
