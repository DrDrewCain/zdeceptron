...
pub fn check(hir: &Hir) -> Result<Site, Vec<TypeError>> {
    ...
    // §4.1 gives each construct one phrasing, and a `Link`'s destination
    // is one slot. What may sit in it is a route value or a URL, and those
    // name disjoint things — a page this program emits, and somewhere
    // outside it — with exactly one overlap: a literal URL this program
    // *does* serve, which would then be expressible both ways. That is
    // refused here, naming the route to write instead.
    //
    // Only enumerable routes are in `seen`, so this decides what §14G.2
    // revision 3 already decided and no more: a URL the compiler rendered.
    let mut destinations = Vec::new();
    for (_, def) in hir.defs.iter() {
        if let DefKind::View(view) = &def.kind {
            literal_destinations(hir, &view.nodes, &mut destinations);
        }
    }
    for (url, span) in destinations {
        let Some((route, _)) = seen.get(&url) else {
            continue;
        };
        errors.push(TypeError {
            message: format!(
                "This program serves `{url}` from the route `{route}`, so the link to it is \
                 written `Link {route}`. A destination has one phrasing (§4.1), and the route is \
                 the one that stays correct when the URL does not."
            ),
            span,
            help: Some(
                "A URL written out is for somewhere outside this program, such as \
...
/// Every `Link` destination written as a text literal, with its span.
///
/// A destination that is not a literal cannot be compared against a
/// rendered URL at compile time, so it is not collected: the rule below
/// is decidable or it is not applied.
fn literal_destinations(hir: &Hir, nodes: &[HirNode], out: &mut Vec<(String, Span)>) {
    for node in nodes {
        match node {
            HirNode::Element(element) => {
                if let Some(expr) = zdc_hir::destination_of(element) {
                    if let HirExprKind::Text(url) = &hir.exprs[expr].kind {
                        out.push((url.clone(), hir.exprs[expr].span));
                    }
                }
                literal_destinations(hir, &element.children, out);
            }
            HirNode::Each(each) => literal_destinations(hir, &each.body, out),
            HirNode::When(when) => {
                for arm in &when.arms {
                    match &arm.body {
                        HirNodeArmBody::Show(element) => {
                            let shown = [HirNode::Element((**element).clone())];
                            literal_destinations(hir, &shown, out);
                        }
                        HirNodeArmBody::Nodes(nodes) => literal_destinations(hir, nodes, out),
                    }
                }
            }
            HirNode::If(conditional) => {
                literal_destinations(hir, &conditional.then, out);
                if let Some(otherwise) = &conditional.otherwise {
                    literal_destinations(hir, otherwise, out);
                }
            }
            HirNode::Scope(scope) => literal_destinations(hir, &scope.body, out),
...
fn constants(hir: &Hir, errors: &mut Vec<TypeError>) -> BTreeMap<DefId, Constant> {
    ...
    // A `static` signal this pass cannot fold is *not* an error. §17.4.8
    // evaluates the build root in a real JavaScript engine, so `1 + 1`,
    // a record construction and a call to a `static`-reachable function
    // are all things the build host computes. The folder here is narrower
    // on purpose: it exists to enumerate a route parameter's `in`, and a
    // signal it cannot fold is reported at the `in` that names it, by
    // `enumeration_of`, in terms of what that `in` needed.
    let _ = &mut *errors;
    known
}
