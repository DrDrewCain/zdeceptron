...
/// The other half of the same rule, and the reason it is not vacuous: a
/// parameter carrying an `in` over a public `static` collection is
/// **trusted**, because the compiler rendered one document per enumerated
/// value and reaching the document is a proof rather than a check
/// (§18.1 semantics 5).
#[test]
fn an_enumerated_route_parameter_may_index_a_trusted_signal() {
    accepted(
        "state slugs is static List of Text starting [\"a\", \"b\"]\n\
         trusted state owners is durable Map of Text to Text starting empty\n\
         route Site\n\
...
/// §14C.3b: a `static` signal is evaluated on the build host, so its
/// initialiser has to be something the build host can evaluate.
#[test]
fn a_static_initialiser_must_be_a_build_time_constant() {
    let message = rejected(
        "state m is static Whole starting 1 + 1\n\
         view\n\
         \x20   Text m\n",
        "not something the build host can evaluate",
    );
    assert!(
        message.contains("foreign function interface"),
        "the help must name what is missing: {message}"
    );
}
...
#[test]
fn link_takes_a_route_and_not_a_string() {
    rejected(
        "route Site\n\
         \x20   Home is \"/\"\n\
         view\n\
