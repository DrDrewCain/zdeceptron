...
/// The same stream, under the name the deploy adapters generate.
///
/// Two names for one endpoint is a cost, and the alternative is worse: the
/// deploy work emits `~watch` and this server answers `live`, so a bundle
/// built for a platform adapter and served by `zdc dev` would subscribe to
/// a URL that 404s — and the failure would look like "live sync does not
/// work" rather than "these two disagree about a path".
///
/// `~` cannot appear in a ZD identifier, so this can never collide with an
/// endpoint name. Whichever spelling survives, both resolve here until one
/// of them is retired.
pub const WATCH: &str = "/_zd/~watch";
...
/// identifier, so — as with [`WATCH`] — this can never collide with an
/// endpoint a program declares.
...
pub fn invocation(path: &str) -> Option<String> {
    ...
    if name.is_empty() || path == LIVE || path == WATCH || path == POLL || path == ATOMIC {
        return None;
    }
    Some(decode(name))
}
...
mod tests {
    ...
    #[test]
    fn the_watch_alias_resolves_to_the_same_stream() {
        // The deploy adapters generate `~watch`; this server was written
        // around `live`. Until one spelling is retired, a bundle built for
        // either has to work against this.
        assert_eq!(invocation(WATCH), None, "the alias is not an endpoint");
        assert!(WATCH.starts_with(PREFIX));
    }
    ...
    fn the_transport_paths_are_not_endpoints() {
        ...
        assert_eq!(invocation("/_zd/~watch"), None);
