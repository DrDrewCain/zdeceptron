...
fn codegen_messages(source: &str) -> Vec<String> {
    ...
    let Ok(types) = zdc_types::check(&hir) else {
        return Vec::new();
    };
...
fn every_diagnostic_codegen_can_raise_is_accounted_for() {
    ...
    // Exactly one refusal is `zdc build`'s alone, and it is the one that
    // says there is no page to build. A second would be the split this file
    // exists to close, growing back one comment at a time.
    assert_eq!(
        build_only.len(),
        1,
        "expected one `// build-only:` refusal — the missing `view` — and found {build_only:?}"
    );
}
