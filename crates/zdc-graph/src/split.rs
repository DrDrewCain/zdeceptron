    ...
    fn orphan_pass(&mut self) {
        ...
        for id in unreached {
            ...
            let ctx = match &self.hir.defs[id].kind {
                ...
                DefKind::Release(_) => Ctx::CLIENT_VIEW,
            };
            ...
            while let Some(at) = frontier.pop() {
                for site in sites_of(self.hir, at) {
                    match site {
                        ...
                        | Site::Bind { .. }
                        | Site::NotAPlace { .. }
                        // A foreign call and a `build` read are values a
                        // platform or the compiler produces, not reads of
                        // another signal, so neither is a derivation edge
                        // either.
                        | Site::ForeignCall { .. }
                        | Site::Build { .. }
                        | Site::Environment { .. } => {}
