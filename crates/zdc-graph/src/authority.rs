    ...
    fn block(&mut self, id: BlockId) -> Flow {
        ...
        for stmt in &self.hir.blocks[id].stmts {
            match stmt {
                ...
                HirStmt::Mutation(_) => {}
                // Nor does a `with` binding, but unlike a mutation its
                // value can reach the result through the name it
                // introduces — so the name is recorded rather than left
                // unbound. Leaving it unbound is not unsound (an unbound
                // local reads `Flow::default()`, which is Untrusted), but
                // it would make every function that spells `with` summarise
                // Untrusted whatever it was given. Bindings are walked in
                // order, so a later value reading an earlier name sees the
                // flow just recorded — the same rule `ifc.rs` applies in
                // the confidentiality direction.
                HirStmt::Bind(bind) => {
                    for binding in &bind.bindings {
                        let flow = self.flow(binding.value);
...
impl<'a> Walk<'a> {
    ...
    fn stmt(&mut self, stmt: &HirStmt) {
        match stmt {
            ...
            // that reached it, which is what `pc` holds — the same rule the
            // `when` and `if` arms below impose on their own binders.
            HirStmt::Bind(bind) => {
                for binding in &bind.bindings {
                    self.expr(binding.value);
                    let flow = self.integrity.flow(binding.value).0.join(&self.pc);
                    self.integrity.bind(binding.local, flow);
                }
            }
    ...
    fn expr(&mut self, id: ExprId) {
        match self.hir.exprs[id].kind.clone() {
            ...
            // `append item to list` raises no obligation of its own — it
            // makes a new list rather than reaching into a declared one —
            // but either operand may contain an index place, so both are
            // walked. Not walking them was a hole: `append x to (orders at
            // i)` would have hidden A1 behind an operation this pass had
