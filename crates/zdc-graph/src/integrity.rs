...
impl<'a> Integrity<'a> {
    ...
    pub fn flow(&self, expr: ExprId) -> (Flow, Option<Grant>) {
        match &self.hir.exprs[expr].kind {
            ...
            // `append item to list` is a derivation, not a grant: the
            // longer list is exactly as trusted as the two values it was
            // made from. No arm here may award a grant it was not given,
            // and this one is given none.
