        ...
        for (root, ctx) in roots {
            ...
            for (def, form) in members {
                match (&self.hir.defs[def].kind, form) {
                    ...
                    (DefKind::Foreign(_), _) => {}
                    // A `release` is discharged at its call sites like the
                    // function it is emitted as, and the rules that make it
                    // a release — REL-ARG, REL-CLOSED, REL-PURE — are the
                    // integrity pass's, not this one's. `solve_function`
                    // answers `Summary::default()` for it, so walking it
                    // here would discharge nothing and say it had.
                    (DefKind::Release(_), _) => {}
                }
            }
        }
