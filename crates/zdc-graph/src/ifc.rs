    ...
    fn nodes(&mut self, nodes: &[HirNode]) {
        for node in nodes {
            match node {
                ...
                // The same rule as `when`, with one condition instead of a
                // scrutinee: whether the branch was taken is observable
                // from the page, so the condition must be public and the
                // `pc` carries its label over both sides.
                HirNode::If(conditional) => {
                    ...
                    let outer = self.pc.clone();
                    self.pc = outer.join(&cond.label.shape);
                ...
                // A component instance's own state. Each local is a signal
                // whose label is its initialiser's, and the body is walked
                // with all of them in scope.
                HirNode::Scope(scope) => {
                    let locals = scope.locals.clone();
                    for local in &locals {
                        let init = self.expr(local.init);
                        self.locals.insert(local.local, init);
                    }
                    let body = scope.body.clone();
                    self.nodes(&body);
                }
                // `children` was replaced by the nodes the call site
                // passed, and those are walked where they were placed.
                HirNode::Children(_) => {}
